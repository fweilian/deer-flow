...
For production or any multi-user deployment, use MySQL:

```bash
# .env
MYSQL_DATABASE_URL=mysql+asyncmy://user:password@mysql:3306/deerflow
UV_EXTRAS=mysql
```
...
Before starting the Gateway, a DBA or migration job must apply both the frozen
application `0001_mysql_baseline` and the independent checkpoint migrations in
`database/mysql/checkpoint/`. The production Runtime only verifies those schemas
and never executes DDL. PostgreSQL is not a supported persistence backend.
