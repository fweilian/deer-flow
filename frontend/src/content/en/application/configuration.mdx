...
For production or multi-user deployments, use MySQL:
...
Set `MYSQL_DATABASE_URL` in your environment, for example `mysql+asyncmy://user:password@localhost:3306/deerflow`.

Before the Gateway starts, a DBA or migration job must apply both the frozen
application `0001_mysql_baseline` and the independent checkpoint migrations in
`database/mysql/checkpoint/`. Production Runtime only verifies those schemas and
never executes DDL. PostgreSQL is not a supported persistence backend.

Install MySQL support for local runs:

```bash
cd backend && uv sync --all-packages --extra mysql
```

For Docker or scripted starts, set `UV_EXTRAS=mysql` before installing or building. The legacy standalone `checkpointer` section is still accepted for compatibility, but prefer `database` for new deployments.
