...
DeerFlow сохраняет нейтральный к провайдерам Channel-фреймворк для будущих пользовательских адаптеров. В этой фазе реестр встроенных провайдеров пуст, поэтому внешние адаптеры сообщений по умолчанию не запускаются.

Пользовательский Channel может регистрироваться через механизм расширений и поддерживать оба направления: преобразовывать входящие события в `InboundMessage`, публикуемые в `MessageBus`, и доставлять `OutboundMessage` через `send()`. Состояние пользовательских подключений остаётся в `channel_connections`, а историческая таблица `webhook_delivery` сохраняется для совместимых расширений.

API Channel в workspace (`/api/channels` и `/api/channel-connections`) остаются доступными. Зарегистрированные адаптеры включаются через `channel_connections.providers` в `config.yaml`; пустая карта provider является допустимым состоянием по умолчанию.
