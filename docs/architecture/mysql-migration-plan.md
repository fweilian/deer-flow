...
> **文档类型**：只读代码调查 + Schema 分析 + 迁移设计
> **调查基线**：`deer-flow` 仓库 HEAD = **`dba975ef`**（`feat_portal` 分支）
...
> 由此产生四个后果：
> ① CheckpointSaver 从"必须自研"改为"**优先复用第三方**"；
> ② 复用的方式是 **直接固定第三方依赖版本**（`langgraph-checkpoint-mysql==3.0.0`，MIT），
>    **默认不 vendor 源码** —— 只有 V5 实测出**必须 patch 的**缺口时才降级为 vendor（§7.4）；
...
| **生效条件** | 🔴 **Gate V5 通过** —— **固定版本依赖** `langgraph-checkpoint-mysql==3.0.0` 的运行语义验收（§7.2 / §7.4 / §13） |
| **V5 未做前的已证实档位** | `Feasible with significant changes` |
| **V5 失败时的处置** | 先判断缺口能否**局部 patch**：能 ⇒ **降级为 vendor**（§7.5，仍不是从零自研）；不能 ⇒ 回到 `Feasible with significant changes` |
...
| 项 | 收缩前 | 收缩后 | 变化 |
| --- | --- | --- | --- |
| **最高风险项** | 自研 MySQL CheckpointSaver（从表结构到全部 SQL 自写） | **固定版本复用第三方 Saver**（`langgraph-checkpoint-mysql==3.0.0`），从零自研降为"仅当 V5 出缺口才 patch" | 🔻 **架构级降险** |
...
| **Checkpoint 持久化** | **Moderate**（原为 Hard） | **固定版本复用上游 Saver** + 连接池接入 + V5 运行语义验收；出缺口才 patch |
...
| **上游代码的长期维护** | **Easy** | 固定版本依赖 ⇒ **零本地 fork**；只有 V5 出缺口才降级 vendor，届时改动面固定为 6 处 import + `UPSTREAM.md`（§7.4/§7.5） |

### 1.4 结论退化条件

出现以下任一条件时，档位回到 `Feasible with significant changes`：

1. **V5 失败且无法局部 patch** —— 固定版本的上游 Saver 存在既不能通过 patch 消除、
   也不能通过子类化绕开的运行语义或 correctness 缺口（§7.5）；
...
>    但同一轮也**实测确认了可行方案**（先对 `threads` 行取真实行锁，§8.6）。
...
因此档位下调**以 V5 为条件**：只有当上游 Saver 的运行语义被实测证实可用时，下调才成立。
...
| **应用表** | **15 张** |
| **应用表列数** | **224 列** |
| **框架表** | **5 张**（`alembic_version` + LangGraph 4 张） |
| **表总计** | **20 张** |
| `sa.JSON` 列 | 20 |
| `DateTime(timezone=True)` 列 | 48 |
| 库层 FK | 2 |
| Partial unique index | 3 |
| 索引（`ix_`/`idx_`/`uq_`） | 47（`ix_` 42 / `idx_` 1 / `uq_` 4） |
...
**C. Goal 0 之后 + 删 `mcp_tasks` / `subagent_batches` / `subagent_batch_items`（本轮目标口径）**
...
| **表总计（含 5 张框架表）** | **17 张** | −3 |
| `sa.JSON` 列 | **11** | −9 |
| `DateTime(timezone=True)` 列 | **29** | −19 |
| 库层 FK | **1** | −1 |
| Partial unique index | **3** | 0 |
| 索引 | **31** | −16 |
| `RETURNING` | **4** | 0 |
| `ON CONFLICT` | **1** | 0 |
| 事务级 advisory lock | **2** | 0 |
| **`with_for_update`** | **28** | **−23（−45%）** |
| **`SKIP LOCKED`** | **2** | **−7（−78%）** |
...
| 3 | `langgraph-checkpoint-postgres` | `runtime/checkpointer/{provider,async_provider}.py` | 🔻 **替换为第三方 `langgraph-checkpoint-mysql==3.0.0`（固定版本依赖，§7.2/§7.4）**；仅当 V5 发现必须 patch 的缺口才降级为 vendor（§7.5） |
...
- 放弃 advisory lock → **真实行锁**（`threads` 行 `SELECT … FOR UPDATE`，§8.2 / §8.6）；
...
> ⚠️ 默认路径（**固定版本依赖**）下这 4 个表名是**上游的、不可改的**
> —— 改名的代价是 **57 处 SQL 字符串 + `LANGGRAPH_OWNED_TABLES` + 一个单测断言**（§6.4），
> 且只有在降级为 vendor 时才**技术上可行**（§7.4 条件 3）。
> ⇒ **默认不改名**，把它列为一条明确的规范例外（§15.12 Q4）。
...
| 🔴 **上游 Saver（checkpoint 4 张表）** | ⛔ **默认路径下改不了** —— 表名硬编码在依赖包的 SQL 字符串里。改动面是：**57 处 SQL 字符串**（`checkpoints` 32 / `checkpoint_blobs` 10 / `checkpoint_writes` 12 / `checkpoint_migrations` 3，分布在 `base.py` / `aio_base.py`）+ `LANGGRAPH_OWNED_TABLES`（`_env_filters.py:30-37`）+ `test_persistence_migrations_env.py:60-63` 的**等值断言**。⇒ 要改必须先满足 §7.4 条件 3 降级为 vendor。**建议不改名**（列为明确的规范例外，§15.12 Q4）—— 收益是风格，成本是永久的本地 diff 与升级冲突 |
...
| `checkpoints` / `checkpoint_blobs` / `checkpoint_writes` | LangGraph checkpoint 三表 | ❌ **默认路径下加不了** —— 上游的 `MIGRATIONS` DDL 字符串是包内代码，不可编辑。要加 COMMENT 需满足 §7.4 条件 3 降级为 vendor（改动面见 §6.4 的 57 处清单）；⚠️ 若降级为 vendor，已建库还需另补一条 `ALTER TABLE … COMMENT` 脚本 |

> 🔻 **本轮变化**：上一版把这条列为"可解"（vendor 后 DDL 字符串可直接编辑）。
> 本轮默认改为**固定版本依赖** ⇒ **该冲突重新成立**，
> 处理方式是把它列为**明确的规范例外**（§15.12 Q4），而不是为了 COMMENT 去 vendor。
> 若组织坚持要 COMMENT，才触发 §7.4 条件 3（代价：永久本地 diff + 每次升级手工重放）。
...
| **引入方式** | ✅ **固定版本依赖** `langgraph-checkpoint-mysql[asyncmy]>=3.0.0,<3.1`（§7.4 默认路径） | §7.4：**不 vendor、不改源码、不 fork** |
...
| **引入方式** | ✅ **固定版本依赖**（`langgraph-checkpoint-mysql[asyncmy]>=3.0.0,<3.1`，写进 `pyproject.toml` 的 `mysql` extra）；⛔ **默认不 vendor**（§7.4） |
...
| 2 | `SELECT_SQL` 用 `json_arrayagg(json_array(..., bl.blob))` 把二进制塞进 JSON（`base.py:169-198`），MySQL 会**自动 base64 编码并加 `base64:type251:` 前缀**，客户端 `decode_base64_blob` 还原（`utils.py:10-14`） | ⚠️ **线路膨胀约 33%**，且 DB 侧需**完整物化**整个 JSON | V5 中做**大 checkpoint 的吞吐/内存实测**（这是性能项，不是正确性项）。**默认路径下 `SELECT_SQL` 不可改**；确需优化 ⇒ 触发 §7.4 条件 1 降级为 vendor（届时直接改 `SELECT_SQL` 并登记进 `UPSTREAM.md`） |
...
> **✅ 第三方 `langgraph-checkpoint-mysql` 3.0.0 在运行语义上满足 DeerFlow 当前 Runtime 的全部需要。**
>
> **上一版否决它的理由是"企业 Schema 规范不匹配"（无 `ag_` 前缀、无 COMMENT、
> 用 JSON/BLOB、索引命名不合规）—— 那些是 Optional Compliance Pass 的关注点，
> 不是运行语义问题。本轮据此推翻该否决。**
>
> 🔴 **本轮同时修正"引入方式"**：上一版把 vendor 当作默认。本轮明确 ——
> **默认直接固定版本依赖**，vendor 只在满足 §7.4 三条明确条件时才使用。
> `orjson` 未被使用 / 表名不合规 / COMMENT 缺失，**都不构成 vendor 的理由**。

### 7.3 🔴 决策树（**本轮修正：先验证原包，再决定是否 vendor**）

```
V5：固定版本依赖 langgraph-checkpoint-mysql==3.0.0 的运行语义验收（§13）
│
├─ 通过 ──► 【默认路径】直接固定版本使用
│            · pyproject 加 [mysql] extra：langgraph-checkpoint-mysql[asyncmy]==3.0.0
│            · adapter：连接池接入 + 配置分支（约 20 行）
│            · 🔴 生产路径**不调用 setup()**：Schema 由运维执行（§11.4）
│            · Runtime 只做 Schema / 版本校验（§11.5）
│            · 依赖面：新增 1 个包 + 驱动（asyncmy / PyMySQL）
│            · ✅ 无本地 fork 维护、无需手工跟踪上游 diff、无需改 import
│
├─ 存在明确的功能或 correctness 缺口 ──► 【降级为 vendor】按 §7.5 处置
│            · 必须先列出证据（哪条语义不满足、哪个测试失败、哪个场景错误）
│            · 且必须满足 §7.4 的三条 vendor 条件之一
│            · 改动量小 ⇒ 直接 patch 并登记；改动量大 ⇒ 才考虑子类化
│            · 档位回到 `Feasible with significant changes`
│
└─ ⛔ 不允许仅因下列理由判定必须 vendor 或必须自研：
     · orjson 是未使用的传递依赖
     · 表名不符合规范（无 ag_ 前缀）
     · COMMENT 缺失
     · 索引命名不合规
...
1. 🔴 **判定"必须自研"或"必须 vendor"时，证据必须是运行语义或 correctness**，不能是风格；
2. 🔴 **不得因为"我们已经写了自研 / vendor 设计"而继续沿用** —— 沉没成本不是理由；
3. 🔴 **不得把"来源是第三方"本身当作风险否决项** —— 它是 MIT、纯 Python、
   依赖只有 `langgraph-checkpoint` + `typing-extensions`（`orjson` 实测**零命中**），
   且**设计目标就是跟随官方 postgres 实现**；
4. 🔴 **顺序不可颠倒**：
   `先验证原包 → V5 通过则直接固定版本使用 → 只有确需 patch 时再 vendor`。
   ⛔ **不要在 V5 之前把第三方源码复制进正式 Runtime 代码。**

### 7.4 接入方式：**优先固定第三方依赖版本**（vendor 仅作降级路径）

**决策（本轮修正）：默认把 `langgraph-checkpoint-mysql==3.0.0` 作为依赖引入并固定版本。
只有满足下面三条之一时，才降级为 vendor 源码进仓库。**

| 维度 | **固定版本依赖（本轮默认）** | vendor 源码（降级路径） |
| --- | --- | --- |
| 依赖声明 | `langgraph-checkpoint-mysql[asyncmy]==3.0.0`（写进 `[mysql]` extra） | 无 |
...
**🔴 只在满足以下三条之一时才 vendor**（否则一律用固定版本依赖）：

| # | 条件 | 判据（必须是可验证的事实，不是感觉） |
| --- | --- | --- |
| 1 | **V5 发现真实的 Runtime / correctness 缺口，且必须 patch 上游源码** | 有可复现的失败用例；且 patch **无法**通过子类化 / 包装实现 |
| 2 | **组织明确禁止引入该第三方 Runtime 依赖** | 有**书面的**依赖准入规则（不是"来源是第三方所以有风险"） |
| 3 | **其内部 SQL / Schema 必须长期改动，且 adapter 无法解决** | 例如公司**强制** checkpoint 表必须加 `ag_` 前缀，且不允许保留别名 |

> ⛔ **明确不构成 vendor 理由的项**：
> `orjson` 是未使用的传递依赖 · 表名不符合规范 · COMMENT 缺失 ·
> 索引命名不合规 · JSON / BLOB 不符合企业规范。
> 这些**要么无害，要么属于 Optional Compliance Pass**。

**为什么默认用依赖**（结合本轮决策优先级）：

1. **"复用成熟实现"首先意味着"走成熟的分发渠道"** —— 固定版本依赖是最低摩擦的复用方式；
...
4. 🔴 **顺序要求**：`先验证原包 → V5 通过则直接固定版本使用 → 只有确需 patch 时再 vendor`。
   **不要在 V5 之前先把第三方源码复制进正式 Runtime 代码。**

> ℹ️ 下面保留完整的 **vendor 路径实施细节**（落点 / 文件集 / import 清单），
> 因为它是**条件 1–3 满足时的既定方案**，且本轮已用真实工具链**验证过该路径可行**
> （6 处 import 改写全部命中、改写后零残留上游 import、改写后的包可正常导入并渲染出
> 22 条 migration）—— 详见 §16 证据清单。
> **但默认路径不是它。**

#### 附 A：vendor 路径的落点与文件集（**仅在 §7.4 三条条件之一满足时执行**）
...
>
> 🔻 **若 §7.4 的三条条件之一被触发而降级为 vendor**：从 extra 里删掉该行，
...
### 7.5 Fallback：缺口出现时的处置（**先 patch，再子类化，最后才重写**）

**仅在 V5 发现明确运行语义 / correctness 缺口时启用。**
⚠️ **进入本节的第一个动作不是 patch，而是判定是否满足 §7.4 的 vendor 条件** ——
条件不满足（例如缺口能用子类化解决）时，**仍然走固定版本依赖**。

#### 处置优先级

| 顺序 | 手段 | 适用条件 | 说明 |
| --- | --- | --- | --- |
| 1️⃣ | **子类化覆写**（**在"固定版本依赖"下就能做，不需要 vendor**） | 缺口集中在少数可覆写的方法 / 钩子上 | `class DeerFlowMySQLSaver(AsyncMySaver)`，只覆写确有缺口的方法。**零 fork 成本，优先考虑** |
| 2️⃣ | **降级为 vendor + 直接 patch 源码** | 缺口在**模块级 SQL 常量**（`SELECT_SQL` / `UPSERT_*`）或私有辅助函数上，子类化够不到；且满足 §7.4 三条条件之一 | 改动量小（< 50 行）；改动登记进 `UPSTREAM.md`。此时"源码保持 pristine"已不成立，patch 最省事、最易读 |
| 3️⃣ | 从零自研 | 缺口是结构性的（表结构本身不可用） | **本轮评估认为该分支已几乎不可能触发**（§7.2 的 22 项验收） |

> 🔴 **顺序修正**：上一版把"直接 patch vendor 源码"放在首位，隐含了"已经 vendor"这个前提。
> 本轮默认不 vendor ⇒ **首位必须是"零 fork 成本"的子类化**；
> 只有当缺口在模块级 SQL 常量上、子类化确实够不到时，才动用 vendor + patch。
...
> ⚠️ **若走子类化**：`BaseMySQLSaver` 把 `MIGRATIONS` / `UPSERT_CHECKPOINT_BLOBS_SQL` /
...
—— 这是本轮相对上一版最大的降险。**但档位仍须回到 `Feasible with significant changes`。**
...
| 位置 | 现语义 | MySQL 替代 |
| --- | --- | --- |
| `runtime/events/store/db.py:148`（事件 `seq` 分配） | `pg_advisory_xact_lock(hashtext(thread_id))` | ✅ **已定**：先对 **`threads` 行**取 `SELECT … FOR UPDATE` 行锁，再读 `max(seq)`（§8.6，**已实测有效**） |
...
"**对一行必然存在的真实行取行锁**" —— 即该 thread 的 `threads` 行：

```python
# runtime/events/store/db.py —— 新增 mysql 分支
if dialect_name == "mysql":
    # ① 先对一行真实存在的行取 X 锁（threads 行必然存在：能写事件的 thread 一定先建过）
    await session.execute(
        text("SELECT id FROM threads WHERE id = :thread_id FOR UPDATE"),
        {"thread_id": thread_id},
    )
    # ② 拿到行锁之后才读 max ⇒ 同一 thread 的并发写者被真正串行化
    return await session.scalar(stmt)
...
| 会话 | 行为 | 结果 |
| --- | --- | --- |
| A | 锁 `threads` 行 → 读到 `max=0` → `SLEEP(4)` → 插 `seq=1` | ✅ 成功 |
| B（1 秒后） | **在 `threads` 行上阻塞约 3 秒** → 解锁后读到 **`max=1`** → 插 `seq=2` | ✅ 成功 |
...
| **新增 per-thread counter / sentinel 表** | ⚠️ 可行，但**需要新表 + 处理锚点创建竞态**。而 `threads` 行**已经存在且语义匹配**（一个 thread 一行）⇒ **优先复用它**（"使用已有能力" > "新增结构"） |
...
```
Goal 0  Runtime 范围清理（删除）          ── 必须先于 Goal 2
Goal 1  MySQL 兼容性与风险 Gate（验证）    ── 与 Goal 0 并行
Goal 2  MySQL 持久化底座（含 Checkpoint）  ── 最高风险，最先做
Goal 3  应用 SQL 与并发迁移
...
### Goal 0（**必须最先做**）：Runtime 范围清理

> **为什么它必须在最前面**：本 Goal 的产出是"**更小的表集合**"。
> MySQL 链的链根 **`0001_mysql_baseline`** 是从当时的 ORM 元数据生成的 ——
> 若这些模块尚未删除，baseline 会把 8 张即将消失的表一起建出来。
> ⇒ **删除必须先于 `0001_mysql_baseline` 的编写。**
...
2. 🔴 **【Gate V1，必须先于 Schema 实施完成】**
   按 **§11.2** 落地 **MySQL 独立链的接线方式** ——
   ① 第二个 `script_location`；
   ...
   **验收标准**：§11.9 的 A 组 8 条 + B 组 5 条。
   ⛔ **不要构建通用 multi-chain framework**：不做链注册表、不做 per-chain 类层次、
   不做 cache manager。**只做"backend 选择对应 script location"这一件事。**
3. 🔴 **【Gate V5，本轮最高优先】** 按 **§7.2 / §7.3** 完成**固定版本依赖**
   `langgraph-checkpoint-mysql==3.0.0` 的运行语义验收：
   **在真实 MySQL 8.0.24 上**跑通 `setup()` + `aput` / `aget_tuple` / `alist` / `aput_writes` / `adelete_thread`，
   并用现有回归测试验证 resume / interrupt / retry / rollback / pending writes /
   多轮长会话 / 并发写 / `checkpoint_channel_mode=full` / asyncmy 池接入。
   **验收结论必须落在 §7.3 决策树的三个分支之一。**
   ⚠️ 同时核对 §7.2 末尾的**三处**实现细节（`INSERT IGNORE` 的取值长度、大 blob 的吞吐/内存、
   `asyncio.get_running_loop()` 的构造约束）。
   🔴 **⛔ 不要在 V5 通过之前把第三方源码复制进正式 Runtime 代码** ——
   默认路径是**固定版本依赖**；只有当 §7.4 的三条条件之一被真实触发时，才降级为 vendor
   （届时才需要 V7 的 `LICENSE` / `UPSTREAM.md`）。
   **必须完成于 Goal 2 的代码之前。**
4. **确定驱动**（§7.7）：`asyncmy` 作为**异步主路径**；`PyMySQL` 作为**同步路径**，
   唯一用途是 `SqlAgentStore`（graph subprocess 的 agent 定义读取）。
   - **验收**：`grep -rn "app_sync_sqlalchemy_url"` 的每个调用点都有明确结论。
5. 🔴 **【Gate V2，已关闭，仅登记】** 上游最终 schema 把 `checkpoint_ns` 的**主键参与**
   换成固定 16 字节的 `checkpoint_ns_hash` ⇒ **索引尺寸与 `checkpoint_ns` 长度无关**（§11.4.3）。
   仅剩 `varchar(2000)` 列宽限制，而 DeerFlow 顶层**始终传 `checkpoint_ns: ""`**。
   ⇒ **无需实测、无需断言兜底。**
6. 🔴 **【Gate V6，已关闭，仅登记】** `JsonMatch` 的 MySQL 分支（§6.5）：
   ⚠️ MySQL `JSON_TYPE()` 返回**大写**（已实测：`INTEGER` / `STRING` / `BOOLEAN` / `NULL` /
   `DOUBLE` / `OBJECT` / `ARRAY`），字面量**必须大写**，否则谓词恒 false（**静默错误**）。
7. **设计 Application / Checkpoint 两份迁移产物**（§11.3 / §11.4）：
   - Application：MySQL 独立链的 `script_location` / `version_table` / 精简 `env.py`；
   - Checkpoint：`database/mysql/checkpoint/` 的**线性 + 版本表守卫** SQL 产物
     （由上游 `MIGRATIONS` 的 22 条**程序化渲染**，不手写）；
   - 明确"由运维 / DBA 执行、Runtime 只读校验"的分工（§11.0）。
8. **设计 Runtime 的只读校验**（§11.5）：6 项校验 + fail closed + 明确报错文案。
9. 把 Checkpointer 的构造从 `provider.py` / `async_provider.py` 的具体类
...
10. 🔴 **直接实现 MySQL 最终路径，不新增并发 abstraction。**
   上一版曾计划抽象 `acquire_txn_lock(key)` / `allocate_sequence(table, col, where)` /
   `conditional_upsert(...)` 并为每种后端提供实现。**本轮取消该计划**，理由：
   ...
   ⇒ **做法**：直接按 §8.2/§8.3 在调用点实现 MySQL 分支（`if dialect == "mysql": …`），
   开发/测试期可以暂时保留 PG branch，但**不把它抽成永久 framework**。
11. 在 CI 中加一条**静态检查**：用 MySQL 方言编译全部 ORM 语句，
    断言不出现 `RETURNING`（§4.9-A 的静默失效面）。
...
1. 在 `harness/pyproject.toml` 的 `[project.optional-dependencies]` 新增 `mysql` extra：
   ```toml
   mysql = [
       "langgraph-checkpoint-mysql[asyncmy]>=3.0.0,<3.1",   # 固定版本（§7.4 默认路径）
       "PyMySQL>=1.1.1",                                     # SqlAgentStore 的同步驱动（§7.7）
   ]
   ```
   - ⛔ **不加 `orjson`**（上游声明但包内零命中）。
   - ⛔ **不 vendor 源码**（除非 §7.4 三条条件之一被触发）。
   - ⚠️ **版本必须精确固定**（`==` 或 `>=x,<x+1`）—— §11.5 的校验项 6 从包内 `MIGRATIONS`
...
18. **决策点**：
    - 若 V5 通过 → 继续；
    - 若 V5 失败 → 按 §7.5 处置：**先子类化 / 先 patch 上游**（改动量大时才 vendor 源码），
      **档位回到 `Feasible with significant changes`**。
...
1. 🔴 **【Gate V3，✅ 已关闭，按已实测的修法落地】** V3 结论为**负面**：
   ...
   **不提供串行化**（两个会话读到同一 MAX，后者 `1062`）；
   ...
   ⇒ **采用已实测有效的修法**：先对 **`threads` 行**取 `SELECT … FOR UPDATE` 行锁，
   再读 `max(seq)`（§8.6）。
...
4. 🔴 **6 处 `("sqlite","postgres")` 能力守卫必须逐处扩展 `mysql`**（否则进程直接拒绝启动）：

   | 位置 | 触发条件 | 失败语义 |
   | --- | --- | --- |
   | `app/gateway/deps.py:84` | `scheduler.multi_instance` | **`SystemExit`** |
   | `app/gateway/deps.py:97` | `GATEWAY_WORKERS` | **`SystemExit`** |
   | `app/gateway/deps.py:132` | `agent_storage.backend: db` | **`SystemExit`** |
   | `app/gateway/health.py:229` | checkpointer 探针 | `ValueError` |
   | `persistence/agents/__init__.py:47` | `agent_storage` 构造 | `ValueError` |
   | `persistence/managed_subagents/__init__.py:32` | 同上 | `ValueError` |

   > 🔻 原 **7 处**，其中 `app/gateway/app.py:405`（`subagent_batches.enabled` 的
   > `RuntimeError`）**随模块删除**（§2.7）。
   > 🔴 **只加驱动不改守卫 ⇒ 进程起不来。**
...
## 13. 实施前 Gate（**本轮状态更新：V3 / V6 / V2 已关闭，V1 / V5 待做**）

| Gate | 内容 | 本轮状态 | 必须完成于 | 不做的后果 |
| --- | --- | --- | --- | --- |
| **V1** | **MySQL 独立链的接线方式** —— ① 第二个 `script_location`；② **不新增缓存抽象**，直接删掉 `_HEAD_REVISION` / `_KNOWN_REVISIONS` 的生产用法（§11.2）；③ 运维执行的 `alembic upgrade head` + Runtime 只读校验（§11.3 / §11.5）。**验收：§11.9 的 A 组 8 条 + B 组 5 条** | 🟡 **待做** | 🔴 **Schema 实施之前（Goal 2）** | 链配置写错会导致 **head 判定串链**、`upgrade` 跑错链 |
| **V5** | 🔴 **固定版本 `langgraph-checkpoint-mysql==3.0.0` 的运行语义验收**（§7.2 / §7.3）—— **在真实 MySQL 8.0.24 上**跑通 `aget_tuple` / `alist` / `aput` / `aput_writes` / `adelete_thread`，用现有回归测试验证 resume / interrupt / retry / rollback / pending writes / 长会话 / 并发写 / `checkpoint_channel_mode=full` / **asyncmy 池接入**；并核对三处实现细节（`INSERT IGNORE`、base64 线路膨胀、`asyncio.get_running_loop()` 的构造约束）。⛔ **不要在 V5 之前把第三方源码复制进正式 Runtime 代码** | 🔴 **待做（最高优先）** | 🔴 **Goal 2 的代码之前** | 误判"可用"→ 上线后才发现 checkpoint 语义缺口；误判"不可用"→ 白白 vendor / 自研 |
| **V3** | `run_events.seq` 在 MySQL 8.0.24 + READ COMMITTED 下的并发分配语义 | ✅ **已关闭（结论为负面，修法已定）**：`SELECT MAX(seq) … FOR UPDATE` **不提供串行化**（两个会话读到同一 MAX，后者 `1062`）；单语句 `INSERT … SELECT` 同样失败；**修法已实测有效** —— 先对 `threads` 行取行锁（§8.6） | ✅ 已完成 | 已识别 ⇒ 直接进入 Goal 3 实施 |
...
| **V7** | **vendor 合规与可维护性** | 🔻 **本轮不再默认适用** —— 默认路径是**固定版本依赖**，**无 vendor、无 LICENSE 副本、无 `UPSTREAM.md`**。**仅当 §7.4 三条条件之一满足而降级为 vendor 时**本 Gate 才生效：① `LICENSE` 全文保留（MIT / `Copyright (c) 2024 Theodore Ni`）；② `UPSTREAM.md` 记录版本 / 发布日 / commit SHA / 逐文件改动状态 / 升级步骤；③ `mysql` extra 相应调整 | 仅在 vendor 时 | 许可合规风险；后续无法重放本地改动 |
...
- **事实**：官方无 MySQL saver（实测只有 base/memory/postgres/serde/sqlite）；
  **`langgraph-checkpoint-mysql` 3.0.0 在运行语义上可用**（§7.2，22 项逐条验收，
  覆盖 DeerFlow 全部真实使用的 5 个异步方法），且**本轮默认以固定版本依赖引入**（§7.4）。
- **为什么仍然危险**：
...
- **缓解**：
  1. **V5 是最高优先 Gate**，必须在 Goal 2 的代码之前完成（§12 Goal 1）；
     **V7 仅在降级为 vendor 时适用**（§7.4 三条条件）；
  2. **保留回退路径**（§7.5）：优先**子类化** `DeerFlowMySQLSaver(AsyncMySaver)`；
     确需改上游行为时才**降级为 vendor 源码 + patch**（改动登记进 `UPSTREAM.md`）
...
| # | 风险 | 严重度 | 可能性 | 缓解 |
| --- | --- | --- | --- | --- |
| 1 | **第三方** Saver（固定版本依赖）的运行语义/性能不达标（V5 未做） | 高（原为极高） | 中 | **V5 验收**；按 §7.5 优先**子类化**，确需改上游行为时才**降级为 vendor + patch**（登记进 `UPSTREAM.md`） |
| 2 | `UPDATE … RETURNING` 编译期不报错 → 运行期才失败 | 高 | **高** | CI 静态检查（MySQL 方言编译 + 断言无 `RETURNING`）+ 逐处手工改写 |
| 3 | **`run_events.seq` 在 MySQL + RC 下失去串行化**（`else` 分支实为 SQLite 语义，且**无重试**） | 高 | 中 | **V3 实测**（§8.6）；改用 per-thread sentinel row + `SELECT … FOR UPDATE`，或唯一约束 + bounded retry |
...
| 12 | 🔻 **Compliance Pass 与第三方 Saver 的冲突** —— 默认路径（固定版本依赖）下 checkpoint 4 张表的**表名 / 索引名 / COMMENT 均不可改**；要改则须触发 §7.4 条件 3 降级为 vendor，且表名改动面是 **57 处 SQL 字符串 + `LANGGRAPH_OWNED_TABLES` + 1 个单测等值断言**，并成为**永久的本地 diff** | 中 | 中 | **默认不改名**，把它列为明确的**规范例外**（§15.12 Q4）。改名与否由组织决策（§16 Q2） |
...
| 15 | 上游代码的 `INSERT IGNORE` 吞掉可忽略错误 | 中 | 低 | V5 中核对目标列（`LONGBLOB` / `JSON` / `VARCHAR(150)`）的取值长度确实不越界；**若确需改为显式错误处理，则触发 §7.4 条件 1 降级为 vendor**（改动登记进 `UPSTREAM.md`） |
| 16 | 上游代码的 `json_arrayagg` base64 线路膨胀影响吞吐 | 中 | 中 | Goal 2 的 2-E 节的大 payload 实测；**若确需改 `SELECT_SQL`，则触发 §7.4 条件 1 降级为 vendor**（登记进 `UPSTREAM.md`） |
...
> **都不进入执行路径**，也**不需要我们维护**。
>
> ⚠️ **仅当 §7.4 三条条件之一满足而降级为 vendor 时**，才需要裁剪：
...
| 2 | advisory lock 替换（🔴 **V3 已实测失败，改为锁 `threads` 行**） | 2 处 |
...
| # | 工作 | 数量 | 与 Core 的冲突点 |
| --- | --- | --- | --- |
| 1 | 表名 `ag_` 前缀 | 12 张应用表 + 4 张 checkpoint 表 | 🟡 checkpoint 表名**改不了**（上游包的 SQL 里硬编码，**默认路径下不可改**）⇒ 要么接受，要么触发 §7.4 条件 3 |
| 2 | 索引命名 `uk_` / `idx_` | 31 个 + checkpoint 表的 4 个 | 🟡 同上 |
| 3 | 中文 TABLE / COLUMN COMMENT | 全量 | ⚠️ checkpoint 4 张表**加不了 COMMENT**（同上） |
| 4 | 未命名唯一约束补名 | 1 处 | 无 |
| 5 | 禁止 `JSON` 列 | 11 列 + checkpoint 3 列 | 🔴 **本轮不采纳**（MySQL 原生可用） |
| 6 | 禁止 `BLOB` / `LONGBLOB` | 2 列 | 🔴 **本轮不采纳**（`LONGBLOB` 是正确容器） |
| 7 | 禁止 FK | 1 处 | 🔴 **本轮不采纳**（InnoDB 原生可用） |

> 🔴 **本轮的关键判断**：第 1–3 项的 checkpoint 部分，是**唯一**可能触发
> "降级为 vendor"（§7.4 条件 3）的场景。**但它不构成 Core Migration 的前置条件** ——
> 按 §2.5，Core 用"能否正确运行"验收，Compliance 用"是否满足规范"验收。
> ⇒ **先按默认路径（固定版本依赖）上线，把表名 / COMMENT 的合规问题留给 Compliance Pass 独立决策。**
...
### 15.8 CheckpointSaver 的接入方案（**默认固定版本依赖**）

```
引入：固定版本依赖 langgraph-checkpoint-mysql==3.0.0（MIT，纯 Python，>=8.0.19）
...
依赖：langgraph-checkpoint-mysql==3.0.0   ✅ 新增（精确固定版本，§7.4 附 C）
      orjson>=3.10.1                      ⚠️ 随上游声明一并安装（包内零命中，无需处理）
      asyncmy>=0.2.10                     ✅ 新增（异步驱动，C 扩展）
      PyMySQL>=1.1.1                      ✅ 新增（SqlAgentStore 的同步驱动，§7.7）
...
验证：V5（运行语义 22 项 + 三处实现细节 + 现有回归测试，在真实 8.0.24 上）
      §11.9 的 A 组（迁移产物）+ B 组（无 DDL 权限账号可正常启动）
```

#### 15.8b 降级路径：vendor 源码进仓库（**仅当 §7.4 三条条件之一满足**）
...
引入：vendor 上游 langgraph-checkpoint-mysql 3.0.0 源码
      ...
      · 改 6 处 import（✅ 本轮已实测：6 处全部命中，改写后**零残留上游 import**）
...
### 15.9 缺口出现时的 fallback 方案（**先子类化，再 vendor+patch，不从零重写**）

```
0️⃣ 前置判定：是否满足 §7.4 的 vendor 条件？
    · 不满足 ⇒ 走 1️⃣（在"固定版本依赖"下就能做，零 fork 成本）
    · 满足   ⇒ 先降级为 vendor，再走 2️⃣

1️⃣ 子类化覆写（默认路径下即可，不需要 vendor）
    class DeerFlowMySQLSaver(AsyncMySaver):
        # MIGRATIONS / UPSERT_*_SQL 是类属性（base.py:246-251）⇒ 直接覆盖
        # SELECT_SQL 走 _select_sql() ⇒ 覆写该静态方法

2️⃣ 降级为 vendor + 直接 patch 源码（改动 < 50 行）
    # 缺口在模块级 SQL 常量 / 私有辅助函数上，子类化够不到
...
3️⃣ 最后：从零自研（本轮评估认为几乎不可能触发）
```

**关键点**：fallback **不再需要**"从零设计表结构 + 重写全部 SQL + 自研 `setup()` 迁移链"。
⇒ 即使 V5 失败，风险与工作量也显著低于上一版的假设。
**但档位必须回到 `Feasible with significant changes`。**

### 15.10 更新后的复杂度结论

| 项 | 结论 |
| --- | --- |
| **目标档位** | **`Moderate`**（条件：V5 通过） |
| **V5 未做前的已证实档位** | `Feasible with significant changes` |
| **V5 失败时的档位** | `Feasible with significant changes`（并启用 §7.5 / §15.9 的 fallback） |
| **性质** | MySQL fresh-cutover / PostgreSQL backend replacement |
| **一句话** | 最难的那件事（自研 CheckpointSaver）从"必须做"变成"**固定版本复用成熟实现**"；**依赖面只增 1 个包**（+ 驱动本身）；范围同时收缩了 38% 的列与 45% 的 `FOR UPDATE`；🔴 **新增一条结构性硬约束** —— **生产 Runtime 零 DDL**，Schema 由运维执行、Runtime 只验证；剩下的是一批**机械但需要仔细**的方言适配与并发验证 |

### 15.11 更新后的 Goal 拆分（**收敛为 6 个**）

| Goal | 内容 | 退出条件 | 前置 Gate | 可独立交付 |
| --- | --- | --- | --- | --- |
| **Goal 0 —— Runtime 范围清理** | 删除 Channel / GitHub Webhook、`mcp_tasks`、`subagent_batches`、LangGraph Store（含 `MemoryThreadMetaStore` 改内部 dict，§7.8 决策 6） | ① Gateway 正常启动；② 普通 MCP 可用；③ 普通 SubAgent `task` 可用；④ 最终 ORM 表集稳定；⑤ 重新扫描 PG 专属依赖 | — | ✅（且必须先于 Goal 2） |
| **Goal 1 —— MySQL 兼容性与风险 Gate** | **V1**（MySQL 独立链）；**V3**（`run_events.seq` 并发实测 —— ✅ 已完成）；**V5**（第三方 Saver 真实运行语义）；**V6**（`JsonMatch` —— ✅ 已完成）；**生产 DDL 权限模型**；**Application / Checkpoint 迁移产物设计** | 上述 Gate 全部有明确结论（通过 / 失败 + 处置） | — | ✅ |
| **Goal 2 —— MySQL 持久化底座** | `asyncmy`；必需的 `PyMySQL`；Application MySQL 连接；**Async CheckpointSaver**；**Runtime Schema / 版本校验**；`0001_mysql_baseline`；**Checkpoint 迁移产物**；Dev/Test 迁移支持 | 🔴 **生产 Runtime 不执行任何 DDL**；§11.9 的 A 组 + B 组验收通过 | **V1 / V5** | ✅ |
...
> **Goal 0 与 Goal 1 可以并行**（前者是删除、后者是验证）。
> **Goal 2 中的 CheckpointSaver 接入必须最先做**（最高风险优先验证）。
> **Goal 5 之后不再保留 PG 兼容分支** —— 不要留"以防万一"的双分支。

### 15.12 剩余真正需要项目负责人确认的问题

> 🔴 **判定标准**：凡是**能通过代码调查、MySQL 8.0.24 实测、并发测试或兼容性测试**回答的，
> **一律不列在这里** —— 那些已在正文给出结论。
> **下面只留真正需要业务 / 组织决策的项。**

| # | 问题 | 影响 | 建议 |
| --- | --- | --- | --- |
| **Q1** | 🔴 **确认 `mcp_tasks` 与 `subagent_batches` 在本期确实没有生产使用计划？** | 决定 −85 列 / −23 `FOR UPDATE` 是否成立 | ⚠️ **需要业务确认**（技术证据已齐：默认关闭 + 无配置启用 + 无调用路径） |
| **Q2** | **是否接受 checkpoint 4 张表不进 MySQL 链、且由运维执行上游派生的迁移产物？** | 决定 Goal 2 的形态与运维流程 | ✅ 建议接受（这是"生产 Runtime 零 DDL"的必然结果） |
| **Q3** | **MySQL 链的 `version_table` 用默认 `alembic_version` 还是 `ag_alembic_version`？** | 必须在 `0001_mysql_baseline` 落地时确定，事后改名要改链根 | ✅ 建议默认 `alembic_version`（两条链在不同 database，天然隔离）；若组织强制 `ag_` 前缀则用 `ag_alembic_version` |
| **Q4** | **是否接受"checkpoint 表名不加 `ag_` 前缀、不加 COMMENT"**（即把 checkpoint 4 张表列为一条明确的规范例外）？ | 若**不接受**，则触发 §7.4 条件 3 ⇒ **降级为 vendor**（永久本地 diff）；若接受，则默认路径成立 | ✅ 建议接受例外 —— 收益是风格，成本是永久升级冲突 |
| **Q5** | **Compliance Pass 的交付时点**（与 Core 同批 / Core 稳定后独立）？ | 决定失败归因是否可分离 | ✅ 建议 Core 稳定后独立交付 |
| **Q6** | **是否接受"本期不新增 Kubernetes / Helm 的 MySQL 部署形态"？** | 决定部署工作范围 | ✅ 建议接受（当前真实目标只有 compose + external DSN） |

> ✅ **本轮已从"待确认"中移除的项**（已由调查 / 实测给出结论，**不再需要负责人拍板**）：
>
> | 原问题 | 现结论 | 依据 |
> | --- | --- | --- |
...
> | `run_events.seq` 能否串行化？ | **不能；修法已实测有效** | §8.6 |
> | `checkpoint_ns` 真实最大长度？ | **不再是问题** —— 主键用固定 16 字节 hash | §11.4.3 |
> | vendor 还是加依赖？ | **默认加依赖**；vendor 仅三条条件 | §7.4 |
...
| **V3 并发实测（负面 + 修法）** | 两独立会话、RC、忠实复现生产语句：`SELECT MAX(seq) … FOR UPDATE` 双方都读到 `@m=0` → 后写者 `1062`；单语句 `INSERT … SELECT IFNULL(MAX(seq),0)+1` **同样 1062**；**修法**：先 `SELECT id FROM threads WHERE id=? FOR UPDATE` → B 阻塞约 3s → 读到 `max=1` → 插 `seq=2`，**两者都成功、序号连续** |
...
| **能力闸门** | 6 处 `("sqlite","postgres")` 判定（§12 Goal 4 的表；原 7 处，`app.py:405` 随 `subagent_batches` 删除） |
...
| 测试 | **113 个**命中 `postgres` 的 `.py` 文件；**8 个 PG 专属文件**（其中 `test_mcp_task_postgres.py` 随模块删除）；**20 个**与 `mcp_tasks`/`subagent_batches` 相关的测试文件 |
