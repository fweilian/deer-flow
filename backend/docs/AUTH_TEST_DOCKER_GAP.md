...
| TC-DOCKER-04 | IM channels use internal Gateway auth | Verify a custom Channel dispatcher attaches the process-local internal auth header plus CSRF cookie/header when calling Gateway-compatible LangGraph APIs | needs `docker logs` |
