...
def test_update_agent_proceeds_on_custom_channel(tmp_path, patched_paths, stub_app_config):
    """Custom Channel runs still allow operator-controlled agent updates."""
    seeded = _seed_agent(tmp_path, description="seeded")

    fake_cfg = AgentConfig(name="test-agent", description="seeded")
    runtime = SimpleNamespace(
        context={"agent_name": "test-agent", "channel_name": "custom"},
