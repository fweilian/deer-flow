...
def test_make_lead_agent_keeps_update_agent_on_channel_runs(monkeypatch):
    """Direct invocation and generic Channel runs still get ``update_agent``.

    Sanity check for the inverse of
    A chat-UI or custom Channel run (or any run with no channel context at all)
    ...
    kwargs_channel = lead_agent_module.make_lead_agent({"configurable": {"agent_name": "test"}, "context": {"channel_name": "custom"}})
    assert "update_agent" in [t.name for t in kwargs_channel["tools"]]
