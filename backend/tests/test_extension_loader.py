...
def test_table_prefix_remains_optional_extension_metadata():
    """Extension migration ownership stays outside DeerFlow's migration chain."""
    assert ExtensionSpec(use=f"{_FIXTURE}:install_ok").table_prefix is None
    assert ExtensionSpec(use=f"{_FIXTURE}:install_ok", table_prefix="ext_").table_prefix == "ext_"
