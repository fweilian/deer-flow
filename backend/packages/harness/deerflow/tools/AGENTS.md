...
4. **Subagent tool** (if enabled):
   ...
   - Direct `create_deerflow_agent` integrations receive cloned `task` tools bound to their explicit `SubagentRuntime`. The bound tool forwards that runtime's exact execution controller and optional caller-owned `AppConfig` into registry/model/tool resolution and `SubagentExecutor`. Keep the original tool name/schema unchanged so model contracts and user-tool deduplication remain stable.
