...
from deerflow.persistence.postgres_schema import validate_store_schema_async as _validate_store_schema
...
async def _async_store_from_database(db_config) -> AsyncIterator[BaseStore]:
    """Async context manager that constructs a Store from DatabaseConfig."""
    ...
    if db_config.backend == "postgres":
        ...
        async with AsyncPostgresStore.from_conn_string(db_config.postgres_url) as store:
            if db_config.schema_init == "manual":
                await _validate_store_schema(store.conn, minimum_version=3)
            else:
                await store.setup()
            logger.info("Store: using AsyncPostgresStore")
            yield store
