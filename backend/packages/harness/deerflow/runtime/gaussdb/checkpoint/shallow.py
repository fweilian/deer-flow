...
from deerflow.runtime.gaussdb_compat import (
    AsyncConnection,
    AsyncCursor,
    AsyncPipeline,
    Capabilities,
...
from deerflow.runtime.gaussdb_compat.pool import AsyncConnectionPool, ConnectionPool

from deerflow.runtime.gaussdb.checkpoint import _ainternal, _internal
...
class ShallowPostgresSaver(BasePostgresSaver):
    ...
    @classmethod
    @contextmanager
    def from_conn_string(cls, conn_string: str, *, pipeline: bool = False) -> Iterator["ShallowPostgresSaver"]:
            ...
            for value in cur:
                checkpoint: Checkpoint = {
                    **value["checkpoint"],
                    ...
                    "pending_sends": [self.serde.loads_typed((t.decode(), v)) for t, v in value["pending_sends"]] if value["pending_sends"] else [],
                }
        ...
        async with self._cursor() as cur:
            ...
            async for value in cur:
                ...
                yield CheckpointTuple(
                    ...
                    checkpoint=checkpoint,
                    metadata=value["metadata"],
                    pending_writes=await asyncio.to_thread(self._load_writes, value["pending_writes"]),
                )
