...
from deerflow.runtime.gaussdb.checkpoint.base import BasePostgresSaver
from deerflow.runtime.gaussdb.checkpoint.shallow import AsyncShallowPostgresSaver
...
class AsyncPostgresSaver(BasePostgresSaver):
    ...
    async def setup(self) -> None:
        ...
        async with self._cursor() as cur:
            ...
            results = await cur.execute("SELECT v FROM checkpoint_migrations ORDER BY v DESC LIMIT 1")
    ...
    async def aput_writes(
        ...
        """
        query = self.UPSERT_CHECKPOINT_WRITES_SQL if all(w[0] in WRITES_IDX_MAP for w in writes) else self.INSERT_CHECKPOINT_WRITES_SQL
        ...
        try:
            ...
            if asyncio.get_running_loop() is self.loop:
                raise asyncio.InvalidStateError(
                    "Synchronous calls to AsyncPostgresSaver are only allowed from a different thread. From the main thread, use the async interface. For example, use `checkpointer.alist(...)` or `await graph.ainvoke(...)`."
                )
        except RuntimeError:
            pass
    ...
    def get_tuple(self, config: RunnableConfig) -> CheckpointTuple | None:
        ...
        try:
            ...
            if asyncio.get_running_loop() is self.loop:
                raise asyncio.InvalidStateError(
                    "Synchronous calls to AsyncPostgresSaver are only allowed from a different thread. From the main thread, use the async interface. For example, use `await checkpointer.aget_tuple(...)` or `await graph.ainvoke(...)`."
                )
        except RuntimeError:
            pass
