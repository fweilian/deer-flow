...
from deerflow.runtime.gaussdb_compat import AsyncConnection, AsyncCursor, AsyncPipeline, Capabilities
...
from deerflow.runtime.gaussdb.checkpoint import _ainternal
from deerflow.runtime.gaussdb.store.base import (
    PLACEHOLDER,
    BasePostgresStore,
    PoolConfig,
    PostgresIndexConfig,
...
class AsyncPostgresStore(AsyncBatchedBaseStore, BasePostgresStore[_ainternal.Conn]):
    ...
    !!! example "Examples"
        ...
        ```python
        from deerflow.runtime.gaussdb.store import AsyncPostgresStore
        ...
        ```python
        from deerflow.runtime.gaussdb.store import AsyncPostgresStore, PoolConfig
    ...
    def __init__(
        ...
        *,
        ...
        if isinstance(conn, AsyncConnectionPool) and pipe is not None:
            raise ValueError("Pipeline should be used only with a single AsyncConnection, not AsyncConnectionPool.")
        super().__init__()
    ...
    async def start_ttl_sweeper(self, sweep_interval_minutes: int | None = None) -> asyncio.Task[None]:
    ...
    async def _batch_put_ops(
        ...
        if embedding_request:
            if self.embeddings is None:
                ...
                raise ValueError(f"Embedding configuration is required for vector operations (for semantic search). Please provide an EmbeddingConfig when initializing the {self.__class__.__name__}.")
    ...
    async def _batch_search_ops(
        ...
        for (idx, _), (query, params) in zip(search_ops, queries, strict=False):
            await cur.execute(query, params)
            rows = cast(list[Row], await cur.fetchall())
            items = [_row_to_search_item(_decode_ns_bytes(row["prefix"]), row, loader=self._deserializer) for row in rows]
            results[idx] = items
