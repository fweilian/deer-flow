...
from deerflow.runtime.gaussdb_compat import Capabilities, Connection, Cursor, Pipeline
from deerflow.runtime.gaussdb_compat.rows import DictRow, dict_row
from deerflow.runtime.gaussdb_compat.types.json import Jsonb
from deerflow.runtime.gaussdb_compat.pool import ConnectionPool
from typing_extensions import TypedDict

from deerflow.runtime.gaussdb.checkpoint import _ainternal as _ainternal
from deerflow.runtime.gaussdb.checkpoint import _internal as _pg_internal
...
""",
        params={
            ...
            "vector_type": lambda store: cast(PostgresIndexConfig, store.index_config).get("ann_index_config", {}).get("vector_type", "vector"),
        },
    ),
    Migration(
        """
...
""",
        ...
        params={
            ...
            "index_params": lambda store: " WITH (" + ", ".join(f"{k}={v}" for k, v in _get_index_params(store)[1].items()) + ")" if _get_index_params(store)[1] else "",
        },
    ),
]
...
class BasePostgresStore(Generic[C]):
    ...
    def _prepare_batch_PUT_queries(
        ...
        if deletes:
            ...
            for namespace, keys in namespace_groups.items():
                ...
                query = f"DELETE FROM store WHERE prefix = %s AND key IN ({placeholders})"
        ...
        if inserts:
            ...
            for op in inserts:
                ...
                if op.ttl is not None:
                    values.append("(%s, %s, %s, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, NOW() + %s::interval, %s)")
                    ...
                    values.append("(%s, %s, %s, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, NULL, %s)")
                ...
                for op in inserts:
                    ...
                    for path, tokenized_path in paths:
                        ...
                        for i, text in enumerate(texts):
                            ...
                            vector_values.append("(%s, %s, %s, %s, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)")
    ...
    def _prepare_batch_search_queries(
        ...
        for idx, (_, op) in enumerate(search_ops):
                ...
                for key, value in op.filter.items():
                    if isinstance(value, dict):
                        for op_name, val in value.items():
                            condition, params_ = self._get_filter_condition(key, op_name, val)
            ...
            extra_filters = " AND " + " AND ".join(filter_clauses) if filter_clauses else ""
            ...
            if op.query and self.index_config:
                ...
                if vector_type == "bit" and cast(dict, self.index_config).get("distance_type") == "hamming":
                    ...
                    if vector_type not in ("vector", "halfvec"):
                        raise ValueError(f"Invalid vector_type for pgvector: {vector_type}")
                    score_operator = score_operator % ("%s", vector_type)

                vectors_per_doc_estimate = cast(dict, self.index_config)["__estimated_num_vectors"]
            ...
            if op.match_conditions:
                for condition in op.match_conditions:
                    if condition.match_type == "prefix":
                        ...
                        params.append(f"{_namespace_to_text(condition.path, handle_wildcards=True)}%")
                    elif condition.match_type == "suffix":
                        conditions.append("prefix LIKE %s")
                        params.append(f"%{_namespace_to_text(condition.path, handle_wildcards=True)}")
                    else:
                        logger.warning(f"Unknown match_type in list_namespaces: {condition.match_type}")
...
class PostgresStore(BaseStore, BasePostgresStore[_pg_internal.Conn]):
    ...
    !!! example "Examples"
        ...
        ```python
        from deerflow.runtime.gaussdb.store import PostgresStore
    ...
    def from_conn_string(
        ...
        *,
        ...
        if pool_config is not None:
            ...
            with Connection.connect(conn_string, autocommit=True, prepare_threshold=0, row_factory=dict_row) as conn:
                if pipeline:
    ...
    def _batch_put_ops(
        ...
        if embedding_request:
            if self.embeddings is None:
                ...
                raise ValueError(f"Embedding configuration is required for vector operations (for semantic search). Please provide an Embeddings when initializing the {self.__class__.__name__}.")
            ...
            queries.append(
                (
                    query,
                    [p for (ns, k, pathname, _), vector in zip(txt_params, vectors, strict=False) for p in (ns, k, pathname, vector)],
                )
            )
    ...
    def _batch_search_ops(
        ...
        if embedding_requests and self.embeddings:
            ...
            for (idx, _), embedding in zip(embedding_requests, embeddings, strict=False):
        ...
        for (idx, _), (query, params) in zip(search_ops, queries, strict=False):
            cur.execute(query, params)
            rows = cast(list[Row], cur.fetchall())
            results[idx] = [_row_to_search_item(_decode_ns_bytes(row["prefix"]), row, loader=self._deserializer) for row in rows]
    ...
    def _batch_list_namespaces_ops(
        ...
        for (query, params), (idx, _) in zip(self._get_batch_list_namespaces_queries(list_ops), list_ops, strict=False):
            cur.execute(query, params)
        ...
        with self._cursor() as cur:
            ...
            if self.index_config:
                ...
                for v, migration in enumerate(self.VECTOR_MIGRATIONS[version + 1 :], start=version + 1):
                    ...
                    sql = migration.sql
                    if migration.params:
                        params = {k: v(self) if v is not None and callable(v) else v for k, v in migration.params.items()}
                        if "dims" in params:
                            try:
                                params["dims"] = int(params["dims"])
                            except Exception as e:
                                raise ValueError(f"Invalid dims for vector index: {params['dims']}") from e
                        if "vector_type" in params:
                            vt = str(params["vector_type"])
                            if vt not in ("vector", "halfvec"):
                                raise ValueError(f"Invalid vector_type for pgvector: {vt}")
                            params["vector_type"] = vt
                        if "index_type" in params:
                            it = str(params["index_type"])
                            if it not in ("hnsw", "ivfflat"):
                                raise ValueError(f"Invalid index_type for pgvector: {it}")
...
def _get_vector_type_ops(store: BasePostgresStore) -> str:
    ...
    if vector_type not in ("vector", "halfvec"):
        raise ValueError(f"Vector type must be 'vector' or 'halfvec', got {vector_type}")
    ...
    if distance_type not in ("l2", "inner_product", "cosine"):
        raise ValueError(f"Vector type {vector_type} only supports 'l2', 'inner_product', or 'cosine' distance, got {distance_type}")
...
def _get_index_params(store: Any) -> tuple[str, dict[str, Any]]:
    ...
    if kind not in ("hnsw", "ivfflat", "flat"):
        raise ValueError(f"Invalid index kind for pgvector: {kind}. Expected 'hnsw', 'ivfflat', or 'flat'.")
...
def get_distance_operator(store: Any) -> tuple[str, str]:
    ...
    if not store.index_config:
        raise ValueError(f"Embedding configuration is required for vector operations (for semantic search). Please provide an Embeddings when initializing the {store.__class__.__name__}.")
