...
DeerFlowは、将来のカスタムアダプター向けにプロバイダーに依存しないChannelフレームワークを保持しています。このフェーズでは組み込みプロバイダーの登録は空で、外部メッセージングアダプターはデフォルトで起動しません。

カスタムChannelは拡張機構から登録でき、入出力の両方を実装します。受信イベントを`InboundMessage`に変換して`MessageBus`へ発行し、`OutboundMessage`は`send()`で配送します。ユーザー接続は`channel_connections`に保存され、履歴の`webhook_delivery`テーブルも互換性のため保持されます。

workspaceのChannel API（`/api/channels`、`/api/channel-connections`）は引き続き利用できます。`config.yaml`の`channel_connections.providers`で登録済みアダプターを有効化します。空のproviderマップも有効なデフォルト状態です。
#### LangSmithトレーシング
