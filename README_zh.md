...
DeerFlow 保留与 provider 无关的通用 Channel 框架，用于未来接入自定义通道。本阶段内置 provider 注册表为空，因此不会默认启动任何外部即时通信适配器。

自定义 Channel 可以通过扩展机制注册，并同时实现上下行：将外部事件转换为 `InboundMessage` 发布到 `MessageBus`，再通过 `send()` 发送 `OutboundMessage`。用户连接状态仍保存在 `channel_connections` 中，历史 `webhook_delivery` 表也会保留以兼容后续扩展。

workspace 的 Channel API 仍可用（`/api/channels`、`/api/channel-connections`）。在 `config.yaml` 的 `channel_connections.providers` 中启用已注册适配器；provider 为空是合法默认状态。
#### 请求链路关联
