...
DeerFlow conserve un framework Channel neutre vis-à-vis des fournisseurs pour les adaptateurs personnalisés. Le registre intégré est vide dans cette phase : aucun adaptateur de messagerie externe n'est démarré par défaut.

Un Channel personnalisé peut être enregistré par le mécanisme d'extensions et gérer les deux directions : convertir les événements entrants en `InboundMessage` publiés sur `MessageBus`, puis envoyer les `OutboundMessage` via `send()`. Les connexions utilisateur restent stockées dans `channel_connections`, et la table historique `webhook_delivery` est conservée pour les extensions compatibles.

Les API workspace restent disponibles via `/api/channels` et `/api/channel-connections`. Activez les adaptateurs enregistrés dans `channel_connections.providers` de `config.yaml`; un registre vide est un état valide par défaut.
#### Traçage LangSmith
