...
DeerFlow retains a provider-neutral Channel framework for optional custom adapters. The built-in provider registry is empty in this phase, so no external messaging adapter starts by default.

A custom Channel can register itself through the extension mechanism and implement both directions: convert incoming transport events into `InboundMessage` objects published to `MessageBus`, and deliver `OutboundMessage` objects through its `send()` method. User-owned connection state remains in `channel_connections`; the historical `webhook_delivery` table is retained for compatible extensions.

The workspace Channel APIs remain available at `/api/channels` and `/api/channel-connections`. Enable and configure registered adapters under `channel_connections.providers` in `config.yaml`; an empty provider map is valid and is the default.
...
immediate. PATs require a database backend (SQLite/MySQL). Full
...
Custom integration skill packs may be projected read-only under the generic integration skill root. Provider-specific credentials remain outside skill metadata and request prompts.
...
Projects require the current database tables and columns. For a MySQL production
deployment, the DBA applies the frozen `0001_mysql_baseline` before Gateway
startup; the Gateway never performs application or checkpoint DDL.
...
- Before upgrading a deployment with `GATEWAY_WORKERS > 1` and `scheduler.enabled: true`, either keep the scheduler on exactly one Gateway worker or configure `scheduler.multi_instance: true` with shared MySQL, `run_ownership.heartbeat_enabled: true`, and `run_events.backend: db`. The upgraded Gateway rejects the unsafe combination at startup instead of starting silently.
